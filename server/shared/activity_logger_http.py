...
class ActivityLoggerHTTP:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if not result.get("success"):
                    logger.warning(f"Activity log failed: {result.get('error')}")
    ...
    async def log_ai_failed(
        ...
        await self.log(
            ...
            entity_title=title,
            source_name=source_name,
            details={"error": error, "model": model},
        )
    ...
    async def log_scrape_started(
        ...
        await self.log(
            ...
            LogCategory.SCRAPE,
            "started",
            f"Scrape started for source: {source_name}",
            entity_type="source",
    ...
    async def log_scrape_completed(
        ...
        await self.log(
            ...
            LogCategory.SCRAPE,
            "completed",
            f"Scrape completed for {source_name}: {docs_found} found, {docs_new} new",
            entity_type="source",
            entity_title=source_name,
            source_name=source_name,
            city_id=city_id,
            details={"docs_found": docs_found, "docs_new": docs_new},
        )
    ...
    async def log_scrape_failed(
        ...
        await self.log(
            ...
            LogCategory.SCRAPE,
            "failed",
            f"Scrape failed for {source_name}: {error[:100]}",
            entity_type="source",
            entity_title=source_name,
            source_name=source_name,
            city_id=city_id,
    ...
    async def log_event_linked(
        ...
        await self.log(
            ...
            source_name=source_name,
            details={"doc_id": doc_id, "doc_title": doc_title},
        )
    ...
    async def log_system_started(
        ...
        await self.log(
            ...
            entity_type="service",
            entity_title=service_name,
        )

    async def log_system_stopped(
        self,
        service_name: str = "SCRAPER",
    ) -> None:
        await self.log(
